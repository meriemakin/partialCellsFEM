...
function [err] = H1errornorm(ng,conn,elem,solution,flag,A,bc1,bc2,xC,yC,radius1,radius2,flagsE,rhs);
...
%	pause
    	...
    	for i =1:ng
			...
			if( ((dist-radius1)<=0.0) && ((dist-radius2)>=0.0) )
                		...
                		if(flag == 0 || flag == 1)
                                        u = analyticalsolutionSinglePoint(A,bc1,bc2,x,y,xC,yC,radius1,radius2,rhs);
                   			err = err + (phi*u_nE - u)^2*jacob*w1*w2; 
                		end
 
                		if(flag == 1)
                   			mat = u_nE'*Dphi*F^-1;
                   			[dudx,dudy] = derivativeAnalyticalsolution(A,x,y,xC,yC,bc2,radius2);
