...
%		pause
		...
		if(hN(j,4) == 1)
		   ...
		   numint = 2; 
		   %get length of curved segment on which the load is applied
		   len = curvelength(xs(1),ys(1),xs(2),ys(2),xC,yC,radius2);
		   lhs = neumannStiffness(minX,maxX,minY,maxY,numberofnodes,xs,ys,xC,yC,len,radius2,numint);
		   %check symmetry norm(lhs-lhs')
		   rhs = neumannForce(minX,maxX,minY,maxY,numberofnodes,xs,ys,xC,yC,len,bc2,radius2,numint);
		   for k = 1:numberofnodes
                   	for l = 1:numberofnodes
                        	s(connElem(k),connElem(l)) = s(connElem(k),connElem(l)) + lhs(k,l);
                   	end
        	   end

        	   for k = 1:numberofnodes
                	f(connElem(k),1) = f(connElem(k),1)  + rhs(k,1);
        	   end
