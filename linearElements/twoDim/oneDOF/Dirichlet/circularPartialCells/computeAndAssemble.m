...
function [solution] = computeAndAssemble(elem,conn,ng,A,rhs,tnn,flagsE,xC,yC,radius,bc,x_n,y_n,bias);
...
%                hN(j,3)
		...
		if(bias == 0)
          		...
          		s(hN(j,1),:)       = zeros(1,tnn);
                	%s
                	%f
          		[w1, w2, w3, w4,rhs] = linearInterpolation(minX,maxX,minY,maxY,x,y,bc);
          		s(hN(j,1),connElem(1)) = w1;
          		s(hN(j,1),connElem(2)) = w2;
          		s(hN(j,1),connElem(3)) = w3;
          		s(hN(j,1),connElem(4)) = w4;
          		f(hN(j,1)) = rhs;
		elseif(bias == 1)
